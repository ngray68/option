...
/**
 * class RestAPIGet3 implements RestAPIAction
 * Executes an HttpGet request over the supplied Session using version 3 of the api
 * The request is passed to the constructor
 * @author nigelgray
 *
 */
public final class RestAPIGet3 implements RestAPIAction {
	
	private final String request;
	
	public RestAPIGet3(String request) {
	...
	public RestAPIResponse execute(Session session) throws SessionException {
		try {
			...
			session.getSessionHeadersVersion3(session.getSessionHeaders()).forEach(get::addHeader);				
